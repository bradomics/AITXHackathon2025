...
def train_hotspot_model(
    *,
    ...
    batch_size: int = 64,
    lr: float = 1e-3,
    pos_weight_collisions: float = 50.0,
    pos_weight_incidents: float = 10.0,
    ...
    from traffic_pipeline.model_hotspot import build_model

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = build_model(n_features=D, n_cells=n_cells, d_model=d_model, arch=arch).to(device)
    opt = torch.optim.Adam(model.parameters(), lr=lr)

    def dense_labels(ptr: np.ndarray, idx: np.ndarray, t: int) -> torch.Tensor:
        y = torch.zeros(n_cells, device=device)
        start = int(ptr[t])
        end = int(ptr[t + 1])
        if end > start:
            y[torch.from_numpy(idx[start:end]).to(device)] = 1.0
        return y
    ...
    for epoch in range(1, epochs + 1):
        ...
        for bi in range(0, len(train_idx), batch_size):
            ...
            loss = _weighted_mse(pred_coll, y_coll, pos_weight=pos_weight_collisions) + _weighted_mse(
                pred_inc, y_inc, pos_weight=pos_weight_incidents
            )
            loss.backward()
            opt.step()
        ...
        with torch.no_grad():
            for bi in range(0, len(val_idx), batch_size):
                if max_val_batches is not None and v_batches >= max_val_batches:
                    break
    ...
    ckpt = {
        ...
        "model_config": {"arch": arch, "d_model": d_model, "n_features": D, "n_cells": n_cells, "context_steps": context_steps},
        "tokenizer_meta": meta,
        "x_mean": X_mean,
        "x_std": X_std,
    }
